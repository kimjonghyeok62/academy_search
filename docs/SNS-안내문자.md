...
[엠스터디수학전문학원 교습비 게시표 예시 (참고)]
신고하신 내용으로 만든 게시표입니다 (내부용·외부용). 틀릴 수 있으니, 다시 한번 확인하시고 사용해 주세요.
https://…/g/a1003-7k2xq9
...
| 회신 주소 · 게시표 주소 | `opts.replyUrl`·`opts.formUrl` | 부르는 쪽이 실어 준다 — `/api/reply-link` 가 둘을 함께 준다 ([학원-회신.md](학원-회신.md)) |
...
## 길이가 넘칠 때 — `buildNoticeSms` 의 네 단계

1. 전체 (설정대로 교습과정 포함)
2. 넘치면 → **교습비 게시표 예시** 블록을 뺀다 (참고 자료라 가장 먼저 덜어낸다)
3. 넘치면 → 빠진 항목이 많은 매체 **3곳**만 `[수정 방법]`·`[관련링크]` 에 남긴다 (`TRIM_KEEP`)
4. 그래도 넘치면 → `[신고하신 교습과정]` 까지 뺀다
