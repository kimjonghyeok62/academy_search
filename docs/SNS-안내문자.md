...
등록번호와 교습비등을 표시하여야 합니다. ( 교육지원청 게시물 : https://www.goegh.kr/… )

[수정 방법]
· 네이버플레이스 → 가격 정보에 교습비 등록(또는 가격표 이미지 첨부)
  소개글에 '등록 제1003호' 기재 (수정방법:https://new.smartplace.naver.com/help/guide?menu=edit)
...
신고하신 내용으로 만든 게시표입니다 (내부용·외부용). 틀릴 수 있으니, 다시 한번 확인하시고 사용해 주세요. (유효기간:10일)
https://…/g/a1003-fz3-7k2xq9
...
| `[<학원명> 관련링크]` | `bucketUrls(result)` | 플레이스는 `currentPlaceUrl`, 나머지는 플레이스 홈에 걸린 링크. 주소를 하나도 못 찾았으면 블록째 뺀다 |

### 순서에는 이유가 있다

무엇이 빠졌나 → 왜 해야 하나(근거·게시물) → 어떻게 고치나 → 얼마인가 → **우리가 본 곳** →
**본보기 게시표** → 언제까지 → 고쳤으면 알려 달라 → 문의.

할 일과 본보기를 먼저 보이고 기한·회신을 그 뒤에 둔다. 기한 뒤에 링크를 늘어놓으면
'언제까지' 가 문자 한가운데 묻힌다. 교육지원청 게시물 링크는 근거를 말한 자리에 함께 두었다 —
맨 아래 관련링크에 두면 학원의 광고 주소와 섞여 '우리 것' 처럼 보인다.

게시표 주소에는 **유효기간(10일)** 을 적는다. 신고 내용이 보이는 자리라 오래 살려 두지 않는데,
언제까지인지 말하지 않고 어느 날 갑자기 안 열리면 학원은 우리 잘못으로 안다. 값은
`FORM_TTL_DAYS` 두 곳(`snsNoticeText.js`·`api/_lib/replyToken.js`)이 같아야 한다.
...
3. 넘치면 → 빠진 항목이 많은 매체 **3곳**만 `[수정 방법]`·`[<학원명> 관련링크]` 에 남긴다 (`TRIM_KEEP`)
