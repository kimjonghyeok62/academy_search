...
export default function NurseryBudgetApp() {
  ...
  function toggleGsSync() {
    if (!gsOn) {
      // Turning ON: Must fetch first
      if (confirm("자동 동기화를 켭니다.\n서버 데이터를 먼저 불러오므로, 저장되지 않은 로컬 데이터는 덮어씌워질 수 있습니다.\n계속하시겠습니까?")) {
        setGsOn(true);
        gsLoad(true); // Fetch immediately
      }
    } else {
      // Turning OFF
      setGsOn(false);
      setIsLoaded(true); // Allow local editing without sync
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
            ...
            <div>
              <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={toggleGsSync} className={`px-3 py-2 rounded-lg border text-sm ${gsOn ? 'bg-green-600 text-white border-green-600' : 'bg-white'}`}>{gsOn ? '자동동기화 ON' : '자동동기화 OFF'}</button>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          <TabButton active={tab === "analysis"} onClick={() => setTab("analysis")} icon={LineChart}>분석</TabButton>
        ...
        {tab === "analysis" && <Analysis expenses={expenses} />}
